// Tests the workspace double-create DEDUPE in ensureHerdrWorkspace
// (src/workspaces.ts). The bug: the dispatcher used to dedupe concurrent same-workspace
// heals in its OWN in-flight map, but the CTO agent called ensureHerdrWorkspace
...
// in-flight map DOWN into ensureHerdrWorkspace so EVERY caller funnels through one
// create per workspace. This drives two concurrent ensureHerdrWorkspace calls against a
...
let dirMod: typeof import("../src/workspaces.ts");
...
beforeAll(async () => {
  ...
  dirMod = await import("../src/workspaces.ts");
  harnessMod = await import("../src/harness.ts");
  originalRunner = harnessMod.getRunner(); // the real herdr backend, restored in afterAll

  // A registered workspace with NO recorded workspace yet, so the heal goes straight to
...
// three methods ensureHerdrWorkspace touches are implemented; the cast keeps the fake
...
test("two concurrent ensureHerdrWorkspace heals share ONE workspaceCreate", async () => {
  const { runner, createCount } = makeFake();
  harnessMod.setRunner(runner);

  // Two heals of the SAME workspace, kicked off together (the CTO-launch-vs-dispatch race).
  const [a, b] = await Promise.all([
    dirMod.ensureHerdrWorkspace(DIR_ID, "/tmp/dedupe-repo", "dedupe"),
    dirMod.ensureHerdrWorkspace(DIR_ID, "/tmp/dedupe-repo", "dedupe"),
  ]);
  ...
  // The persisted workspace row reflects that one workspace.
  const row = dbMod.db
    .query<{ herdr_workspace: string | null }, [string]>(
      `SELECT herdr_workspace FROM workspaces WHERE id=?`,
    )
    .get(DIR_ID)!;
