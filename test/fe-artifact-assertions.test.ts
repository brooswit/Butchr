...
function fixture(
  name: string,
  opts: { tokenDefs?: number; sprite?: boolean; devReact?: boolean; empty?: boolean } = {},
): string {
  ...
  writeFileSync(
    join(dir, "index-abc123.js"),
    opts.devReact ? 'console.log("react-dom.development");' : 'console.log("hi");',
  );
  return dir;
}
...
describe("scripts/assert-fe-artifact", () => {
  ...
  test("silent-failure 3: React's development bundle in the output fails", () => {
