...
//
// >>> THIS FILE COVERS THE VANILLA DIFF READER, WHICH IS STILL THE ONE THAT SHIPS. <<<
// Phase 4b landed the React `DiffView` (public/views/diff.tsx, covered by test/diff-view.test.ts)
// but could not mount it: the diff has no route of its own, and its only caller is `renderTask` in
// the still-vanilla views/task.js. So both renderers exist, both are tested, and they share ONE
// tokenizer and ONE line-anchor in views/diff-logic.ts — which is what stops them drifting. Phase 4c
// rewrites task.js in React, deletes views/diff.js, and deletes this file with it.
//
// It was renamed from test/diff-view.test.ts to make room for the React one. Nothing else changed.
//
