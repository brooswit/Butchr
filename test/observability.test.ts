    ...
    .query(
      `INSERT INTO tasks
         (id, workspace_id, status, blocked_by, kind, tags, priority, plan_preview,
          version_bump, idle, has_agent, question, summary, last_dispatch_error, created_at)
       VALUES (?, ?, ?, '[]', 'task', '[]', 0, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      id,
      ...
      o.idle ? 1 : 0,
      // has_agent is the honest launched-agent marker (attentionList's idle gate keys on it).
      o.pane ? 1 : 0,
      o.question ?? null,
      o.summary ?? null,
