...
// (the only writer that REBINDS them) come across with it. Nothing else could move it — a signature
// change to pass the comments in would have rewritten its call site in views/task.js, and this phase
// is "moves, no logic."
//
...
// views/diff.js, exactly as core/state-meta.js's applyStateMeta does for its tables.
//
...
export let inlineComments = new Map(); // key -> { path, line, side, ctx, text }
...
export let collapsedDiffFiles = new Set();

export function resetInlineComments(taskId) {
