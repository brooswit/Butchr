...
import type { CeoStatus, InitiativeView, Project, Repo, TerminalResult, Workspace } from "../core/types.js";
    ...
    <div className="panel ceo-card">
      ...
      <div className="ceo-row">
        ...
        {/* Unlike the CTO button (which HIDES when not running), this stays visible and disables
            ...
            THE `title` RIDES ON A WRAPPING <span>, NOT ON THE BUTTON. LaunchPad's `ButtonProps` has
            no `title` (react-aria types the prop set and a native tooltip is not in it), so
            `ActionButton` deliberately does not forward one — see components/button.tsx. The wrapper
            also fixes the bug that matters here: a browser suppresses hover events on a DISABLED
            button, so the hint explaining WHY it is disabled was the one least likely to appear.
            The swimlanes' leader-terminal button already does exactly this. */}
        <span title={term.title}>
          <ActionButton
        ...
        {s.overridden ? (
          <span title="Clear the per-project override and inherit the global default">
            <ActionButton
...
function DeleteProjectModal({
  ...
  const { run, pending } = useAction(
    async () => {
      ...
      // `(data && … && data.error) || res.statusText` reads fine and does not TYPE: the left arm of
      // `data && …` widens to `{}` for an `unknown` operand, so `msg` is `string | {}` and neither
      // `setError` nor `new Error` accepts it. Narrow explicitly.
