...
// called. test/projects-initiatives-ui.test.ts and the pure half of test/projects-ceo-ui.test.ts
// import this leaf directly.
//
// `ceoNote` used to be the odd one out — DOM-free logic that returned a NODE, so it had to stay in
// the view. In Phase 4 it returns JSX, which is still a node, so it still lives in projects.tsx.
import type { CeoStatus, InitiativeView, Project } from "../core/types.ts";

/** The three-way pill the OVERVIEW card shows, derived from the LIST row's `ceo_enabled` ALONE.
 *  The overview must not assert a resolved on/off it cannot know — the fully-resolved state
 *  (enabled/globalGate/live) belongs to the detail card via `GET /api/projects/:id/ceo`. */
...
// The project-level "X/Y initiatives done" rollup for the overview card — counts DONE initiatives
// using the server's authoritative `done` boolean on each InitiativeView. Only cross-repo
// initiatives appear in the list (single-repo ones are ungrouped), so this counts those.
