...
//
// This module is DOM-FREE at load (nothing here touches `document`), so its tests
// (test/graph-hierarchy.test.ts, test/graph-rollup-completion.test.ts, test/swimlane-order.test.ts)
// import it directly and assert on the real exports. These helpers used to be fenced by
// `<test-extract:...>` sentinels and eval'd out of app.js with `new Function`, because a classic
// script could not be imported. That harness is gone — do not reintroduce a sentinel here.
//
...
import { TERMINAL_STATUSES } from "./state-meta.js";
...
export function pruneWorkCaches(liveIds, expanded, activity) {
...
export const COMPLETE_STATUSES = new Set(["merged", "rolled_back", "done"]);
export function isCompleteStatus(status) { return COMPLETE_STATUSES.has(status); }
...
export function workListPath(workspaceId) {
...
export function workLeaves(work) {
...
export function isHistoryItem(w) {
  if (!w) return false;
...
export function reverseDeps(tasks) {
  const m = new Map();
...
// itself is excluded.
export function gatedSubtree(rootId, dependentsOf) {
  const seen = new Set();
...
export function graphLevels(nodeIds, edges) {
  const level = {};
...
// The graph's containment (and S3's future grouping) derives from this ONE set of rules, so the
// views and the tests can't drift.
...
export function graphChildOf(w) { return w && (w.parent_id || w.story_id); }
...
export function storyMemberIds(storyId, ids, byId) {
  const out = [];
...
export function storySubtaskTotal(counts) {
  const c = counts || {};
