...
export const TEMPLATES: Template[] = [
  {
    ...
    body: [
      ...
      "surrounding module's style and import with the `.ts` extension. If this changes a",
      "public surface (a REST route, SSE event, config/env var, DB column, task state,",
      "MCP tool, or CLI command), update the relevant section of CONTRIBUTING.md in the",
      "SAME change — butchr records the CHANGELOG entry + version bump at merge from your",
      "request_review summary, so do NOT hand-edit CHANGELOG.md / package.json.",
      "",
  ...
  },
  {
    ...
    body: [
      ...
      "observable behavior needs no docs edit — but if any public surface (a route, SSE",
      "event, config var, or DB column) moves, update the relevant CONTRIBUTING.md",
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "READ FIRST: public/app.js, public/index.html, public/style.css, and the webapp",
      ...
      "contract; if you need data the API does not expose yet, add the route first.",
      "",
      "FOLLOW CONTRIBUTING.md: zero new dependencies; match the hash-routed, SSE-driven",
      "style of app.js and the existing style.css conventions. If the new view consumes a",
      "new route or SSE event, note it in the relevant CONTRIBUTING.md section.",
      "",
  ...
  },
  {
    ...
    body: [
      ...
      "both green before calling request_review. If the reverted change altered a public",
      "surface CONTRIBUTING.md documents, update the relevant section to match.",
      "",
      "FOLLOW CONTRIBUTING.md: zero new dependencies (Bun stdlib only); match the",
  ...
  {
    name: "add-endpoint",
    description: "Add a REST endpoint: thin handler over a service function, with docs + a test.",
    body: [
      "Add a REST endpoint: {{method}} {{path}}.",
      "",
      "READ FIRST: src/server.ts (route registration + the json/readJson/HttpError",
      "helpers + the route table), the relevant service module, and CONTRIBUTING.md",
      "(\"How to add things\").",
      "",
      ...
      "",
      "FOLLOW CONTRIBUTING.md: zero new dependencies. If it should be drivable from the",
      "shell, add a bin/butchr subcommand that maps onto exactly this route (no extra",
      "server logic).",
      "",
