...
- `herdr` on `PATH`, **with its server running** (`herdr server`, or launch the
  ...
  and `bun test` without a live herdr (tests stub it out — see §6).
...
2. **Update SPEC.md (and CHANGELOG/version where the project tracks them) in the
   same change.** SPEC.md is the living design doc — if you change behavior,
   config, the data model, or an interface, reflect it there so the docs never
   lag the code.
